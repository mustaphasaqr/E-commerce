...
public class EmailVerificationTokenRepositoryImpl implements EmailVerificationTokenRepository {
    ...
    public EmailVerificationTokenRepositoryImpl(RedisTemplate<String, Object> redisTemplate, ObjectMapper objectMapper) {
    ...
    @Override
    public Optional<EmailVerificationToken> findByToken(String tokenValue) {
        String key = KEY_PREFIX + tokenValue;
        Object value = redisTemplate.opsForValue().get(key);
        return Optional.ofNullable(value).map(this::convertToEmailVerificationToken);
    }

    private EmailVerificationToken convertToEmailVerificationToken(Object value) {
        ...
        throw new IllegalStateException("Unexpected type for EmailVerificationToken: " + value.getClass());
    }
    ...
    public void deleteAllByUserId(UserId userId) {
        ...
        if (keys != null && !keys.isEmpty()) {
            for (String key : keys) {
                Object value = redisTemplate.opsForValue().get(key);
                if (value != null) {
                    EmailVerificationToken token = convertToEmailVerificationToken(value);
