...
@DisplayName("EmailVerificationToken Domain Model Tests")
class EmailVerificationTokenTest {

    @Test
    @DisplayName("Should create email verification token with valid data")
    void shouldCreateEmailVerificationTokenWithValidData() {
        String userId = "USER-123";
        String email = "test@example.com";

        EmailVerificationToken token = EmailVerificationToken.create(userId, email);
    ...
    void shouldReconstituteTokenFromPersistence() {
        ...
        EmailVerificationToken token = EmailVerificationToken.reconstitute(
    ...
    void shouldMarkTokenAsUsed() {
        EmailVerificationToken token = EmailVerificationToken.create("USER-123", "test@example.com");
    ...
    void shouldBeEqualBasedOnTokenValue() {
        ...
        EmailVerificationToken token1 = EmailVerificationToken.reconstitute(
            tokenValue, "USER-1", "test1@example.com",
            LocalDateTime.now(), LocalDateTime.now().plusHours(24), false, null
        );
        EmailVerificationToken token2 = EmailVerificationToken.reconstitute(
